    ...
    <div className="w-full max-w-5xl mx-auto pb-16 px-4 sm:px-6 lg:px-8 mt-6">
      ...
      <div className="w-full mb-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-8">
          ...
          {/* Countdown Timer */}
          {timeLeft && (
            <div className="flex flex-col items-start sm:items-end gap-1.5 shrink-0 bg-white p-3.5 rounded-2xl border border-slate-100 shadow-sm">
        ...
        <div className="relative w-full max-w-sm">
          ...
          <input
            ...
            className="block w-full pl-10 pr-4 py-2.5 text-sm border border-slate-200/60 rounded-xl leading-5 bg-white shadow-sm placeholder-[#b4aee8] focus:outline-none focus:ring-2 focus:ring-[#8c7df0] focus:border-transparent font-medium text-[#2b1f5e] transition-all"
          />
        </div>
      </div>

      {error ? (
        <div className="flex flex-col items-center justify-center h-64 text-slate-500 bg-white rounded-3xl shadow-sm">
          <AlertCircle className="w-12 h-12 text-red-400 mb-4" />
          <p className="font-medium text-lg">{error}</p>
        </div>
      ) : loading ? (
        <div className="space-y-4 pt-2 bg-white rounded-3xl p-8 shadow-sm">
          <div className="h-64 bg-slate-100 rounded-3xl animate-pulse mb-8"></div>
          {[...Array(5)].map((_, i) => (
            <div key={i} className="flex items-center gap-4 p-5 rounded-3xl bg-slate-50 animate-pulse">
        ...
        <div className="flex flex-col items-center justify-center h-64 text-slate-500 bg-white rounded-3xl shadow-sm p-8">
      ...
      {!loading && !searchQuery && !isCurrentUserVisible && myRankInfo?.rank && (
        <motion.div 
          ...
          className="fixed bottom-20 left-1/2 transform -translate-x-1/2 w-full max-w-[90%] sm:max-w-md bg-white border-2 border-[#8c7df0] p-4 rounded-2xl shadow-2xl flex justify-between items-center z-50"
        >
          <div>
