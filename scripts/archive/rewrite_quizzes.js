...
@keyframes slideOutLeft {
  ...
  20% { transform: translateX(-20px) rotate(-2deg) scale(0.98); }
  100% { transform: translateX(-110%) rotate(-8deg) scale(0.85); opacity: 0; }
}

@keyframes slideInRight {
  0% { transform: translateX(110%) rotate(8deg) scale(0.85); opacity: 0; }
  60% { transform: translateX(-8px) rotate(-1deg) scale(1.01); opacity: 1; }
  80% { transform: translateX(4px) rotate(0.5deg) scale(1); }
...
.animate-pulse-custom { animation: pulse 1.5s ease infinite; }
.animate-fadeUp { animation: fadeUp 0.3s ease forwards; }
.animate-slideOutLeft { animation: slideOutLeft 0.45s cubic-bezier(0.4, 0, 0.2, 1) forwards; }
.animate-slideInRight { animation: slideInRight 0.5s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
.animate-burst { animation: burst 0.6s ease forwards; }
.animate-flash { animation: flash 0.4s ease forwards; }
.animate-shake { animation: shake 0.5s cubic-bezier(0.36,0.07,0.19,0.97); }
.animate-confetti { animation: confetti var(--duration) linear forwards; animation-delay: var(--delay); }
`;

let globalsCss = fs.readFileSync('app/globals.css', 'utf8');
if (!globalsCss.includes('@keyframes shake')) {
...
import { useGameState, markLessonDone } from "@/lib/progress";
...
export default function QuizzesReadPage() {
  ...
  // Ensure we only show 10 questions for the progress bar
  ...
  const handleNextQuestion = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        setAnimState('idle');
      }, 500); // slideInRight duration
    }, 450); // slideOutLeft duration
  };
  ...
  if (isFinished) {
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 bg-[#F8F9FC] relative overflow-hidden h-full min-h-[600px]">
        {score >= 8 && (
  ...
  return (
    <div className="flex-1 bg-[#F8F9FC] p-10 min-h-screen relative overflow-hidden">
      {/* Screen Flash Overlay */}
      {showFlash && (
        <div className="fixed inset-0 z-[300] pointer-events-none bg-[rgba(34,197,94,0.08)] animate-flash" />
      )}

      {/* Top Bar */}
      <div className="max-w-[800px] mx-auto flex items-center justify-between mb-[12px] py-[16px]">
      ...
      {/* Progress Bar */}
      <div className="max-w-[800px] mx-auto flex gap-[6px] mb-[32px]">
      ...
      <div className={\`max-w-[720px] mx-auto \${cardAnimClass}\`}>
        {/* Card */}
        <div className="bg-[linear-gradient(135deg,#EEF3FF,#F8F9FC)] border border-[#C7D7FF] rounded-[20px] px-[36px] py-[32px] shadow-[0_4px_20px_rgba(61,82,160,0.08)]">
          ...
          <h2 className="text-[18px] font-[700] text-[#111111] leading-[1.6]">
             {currentQuestion?.question}
          </h2>
        </div>

        {/* Answer Options */}
        <div className="mt-[20px] flex flex-col gap-[10px] relative">
           {currentQuestion?.options.map((opt: string, idx: number) => {
             ...
             return (
               <div 
                 ...
                 className={\`relative border-[1.5px] rounded-[12px] px-[20px] py-[14px] flex items-center gap-[14px] cursor-pointer transition-all duration-200 ease-out \${hoverClass} \${disabledClass}\`}
               ...
               >
                 ...
                 {hasAnsweredCurrent && isCorrect && <Check className="ml-auto text-[#22C55E]" size={20} />}
                 {hasAnsweredCurrent && isSelected && !isCorrect && <XIcon className="ml-auto text-[#EF4444]" size={20} />}
        ...
        {hasAnsweredCurrent && (
          <div className="flex justify-end mt-[16px] animate-fadeUp">
             <button onClick={handleNextQuestion} className="bg-[#3D52A0] hover:bg-[#2e3e78] text-white rounded-[8px] px-[28px] py-[12px] font-[700] text-[14px] shadow-sm transition-colors">
...
console.log('Quizzes read page rewritten successfully.');
