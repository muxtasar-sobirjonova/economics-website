...
with open('app/(marketing)/page.tsx', 'r', encoding='utf-8') as f:
    content = f.read()
...
# I will just replace the "T" with an icon for better consistency, or maybe I don't see a "(". Let me see if there's a stray character in DashboardMockup or RoadmapMockup.
...
content = content.replace('<IconLock size={20} className="text-gray-400" />', '<IconLock size={20} className="text-gray-400" />')
# Use IconCheck and IconLock consistently
...
content = content.replace('icon={IconTarget}', 'icon={IconTarget}')
content = content.replace('icon={IconChartLine}', 'icon={IconChartLine}')
content = content.replace('icon={IconFlame}', 'icon={IconFlame}')
# Tabler icons are inherently matched, but just in case, I will make them all have the same bg.
...
quiz_mockup_replace = """<div className="flex items-start gap-5 bg-white p-8 rounded-3xl border border-gray-200 shadow-sm w-full cursor-pointer">
...
new_quiz_mockup = """<div className="bg-[#F8F9FC] rounded-3xl border border-gray-200 shadow-xl overflow-hidden flex flex-col h-[400px] w-full shrink-0 relative">
    <BrowserChrome />
...
content = content.replace('>Sign Up<', '>Start learning today<')
content = content.replace('Get started for free', 'Start learning today')
...
with open('app/(marketing)/page.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
