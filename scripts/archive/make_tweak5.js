...
// 1. Update the wrapper container for isLesson1
...
// Update for the default return block as well
content = content.replace(
    "width: '100%', marginRight: '300px'",
    "width: marginOpen ? '70%' : '100%', paddingRight: marginOpen ? '0' : '48px', transition: 'width 0.3s ease, padding 0.3s ease'"
);
...
// Safely replace the FIXED RIGHT PANEL block by splitting
let parts = content.split('{/* FIXED RIGHT PANEL */}');
if (parts.length === 3) {
  let endIdx1 = parts[1].indexOf('    </div>\\n  );');
  if (endIdx1 === -1) {
    // try different search
    endIdx1 = parts[1].indexOf('        </div>\\n      </div>\\n    );');
    if (endIdx1 === -1) endIdx1 = parts[1].indexOf('</div>\\n      </div>\\n    );');
    if (endIdx1 === -1) endIdx1 = parts[1].indexOf('</div>\\n    </div>\\n  );');
    if (endIdx1 === -1) endIdx1 = parts[1].lastIndexOf('</div>\\n      </div>\\n    );');
    
    // Actually, look at page_current.tsx:
    //         </div>
    //       </div>
    //     );
    //   }
  ...
  content = content.replace(
      /\{\/\*\s*FIXED RIGHT PANEL\s*\*\/\}.*?<\/div>\s*<\/div>\s*\);\s*\}/gs, 
      "\\n" + newLayout + "\\n      </div>\\n    );\\n  }"
  );
  
  // For the default return block
  content = content.replace(
      /\{\/\*\s*FIXED RIGHT PANEL\s*\*\/\}.*?<\/div>\s*<\/div>\s*\);\s*\}/gs, 
      "\\n" + newLayout + "\\n      </div>\\n    );\\n}"
  );
  
  // Wait, the regex replace above might fail if the end tag doesn't match perfectly. 
  // Let's just do a reliable string split and replace
}

// Write the file
fs.writeFileSync('C:/Users/user/.gemini/antigravity-ide/scratch/economics_website/tweak5.js', \`
const fs = require('fs');
