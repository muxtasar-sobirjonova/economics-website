...
function compact_agenda() {
    ...
    const old_block_start = '            return (\n              <div\n                key={item.id}';
    const old_block_end = '              </div>\n            );\n          })\n        ) : (';
    
    if (content.includes(old_block_start) && content.includes(old_block_end)) {
        const start_idx = content.indexOf(old_block_start);
        const end_idx = content.indexOf(old_block_end) + '              </div>\n            );'.length;
...
function compact_mistakes() {
    ...
    const old_caught_up = `  if (count === 0) {
    return (
      <div
        id="review-mistakes"
    ...
    const new_caught_up = `  if (count === 0) {
    return (
      <div
        id="review-mistakes"
      ...
      >
        <div className="flex flex-col items-center text-center">
          <h3 className="text-[#1A1A3E] font-[700] text-[16px] tracking-tight mb-1">
            {justReviewedAll ? "All done! 🎉" : "No Mistakes to Review"}
          </h3>
